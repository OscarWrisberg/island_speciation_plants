---
title: "All_species_mono_phylo_islands"
...
sum(unlist(output_as_df_all_sp[[1]]$radiating_sp)) + sum(unlist(output_as_df_all_sp[[1]]$colonization_sp))
...
I need to do some fiddling with finding some means mins and max across the tacted trees.
...
output_all_sp_congregated
```




Now I will start plotting and making models!
...
logo_phyl <- readPNG(file.path("./","Phylo_for_plot.png")) %>% 
  image_read()
bitmap <- logo_phyl[[1]]
bitmap[4,,] <- as.raw(as.integer(bitmap[4,,])*0.2)
logo_phyl <- image_read(bitmap)

p23 <- ggdraw() +
  draw_plot(p2) +
  draw_image(logo_phyl, scale = 0.25, hjust = -0.3, vjust = -0.37)
...
Now lets make some models for this stuff
...
saveRDS(output_all_sp_test, "Output_all_sp_test.rds")
...
```
Now I am running the models for the anagenetic speciation.
...
```
Now I want to run the same analysis but instead of using the mean colonization i want to fit the model to each of the 100 tacted trees
```{r}
# Creating a for loop which fits the model using all 100 different datasets 
# -------- Anagenesis ----------------
...
```
And now we will find the confidence intervals for all these models and make some plots of them
...
What if we only look at botanical countries with Volcanic origin?
I Think isolation would be more important if we exclude all the continental islands from the model
That means we would probably loose all the really big islands, but does that matter?
...
```
Very interestingly, area is still significant, but max elevation is also significant and has a much larger effect on the amount of radiating species on the island. 
...
Now let us create the same plot for the anagenetic speciation.
...
Now that we have created these spacy violin plots, all that is left is combining these plots into one plot showing the differences in coefficient estimates for anagenetic and cladogenetic speciation. 
```{r}
...
prow
```
Something important to note when looking at this graph.
The reason that we have no estimates for cladogenesis for the coryphoids is that we simply do not have enough samples from each category.
And we have no estimate for cladogenesis on continental island botanical countries because the continental islands are used as the reference for the rest. and the reason why we do have a estimate for Continental islands for anagenetic speciation is because all the atolls are being used as a reference. 
