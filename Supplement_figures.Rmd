---
title: "Supplement_figures_Island_paper"
...
Run this script after you have run the main analysis.
...
load(file.path("../data","/workspace_TACTed_species.RData"))
output_with_island_no_log <- readRDS(file.path("./","output_for_test_all_sp"))
...
load(file.path("../data","/workspace_TACTed_species2.RData"))
...
```{r}
# Part where I find some details about the dataset

```



Creating all the CSV files which are in the supplement.
```{r}
...
plot_col_hump <- ggplot(data = output_all_sp_congregated, aes(x = dist, y = colonization_sp_mean + radiating_nodes_mean)) +
  geom_point(aes(color = GeologicalOrigin), size = 3.5) +
  xlab("Isolation") +
  ylab("Anagetic speciation events") +
...
Now I want to do the same thing but with only endemic species.
```{r}
